...
DEFAULT_OPENFLIGHTS_ROUTES_PATH = Path(__file__).resolve().parents[3] / "data" / "routes.dat"
...
# Defunct airline codes that should be excluded
DEFUNCT_AIRLINE_CODES = {"9W", "G8"}

# Airline priority list - prefer known/reliable carriers in priority order
AIRLINE_PRIORITY = [
    "6E",  # IndiGo
    "AI",  # Air India
    "QP",  # Akasa Air
...
def _choose_airline_name(airline_codes: List[str]) -> str:
    """Choose airline name preferring known/reliable carriers in priority order."""
    available = set(airline_codes)
    
    # Prefer carriers in our priority list, in priority order
    for code in AIRLINE_PRIORITY:
        if code in available and code in AIRLINE_CODE_TO_NAME:
            return AIRLINE_CODE_TO_NAME[code]
    
    # Fall back to any remaining code we have a name for
    ...
    if airline_codes:
        return _format_airline_name(sorted(airline_codes)[0])
    ...
    if OPENFLIGHTS_ROUTES_PATH and os.path.exists(OPENFLIGHTS_ROUTES_PATH):
        with open(OPENFLIGHTS_ROUTES_PATH, "r", encoding="utf-8") as handle:
            ...
            for row in reader:
                ...
                # Skip defunct airline codes when building pair_airlines
                if airline_code and airline_code not in DEFUNCT_AIRLINE_CODES:
