...
export default function NavBar() {
  ...
  return (
    <header className="h-12 shrink-0 flex items-center px-3 sm:px-4 gap-2 sm:gap-3 relative z-[60] border-b border-white/5 bg-surface-container-lowest/95 backdrop-blur-xl">
      <Link
        href="/"
        onClick={resetSearch}
      ...
      >
        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-primary/30 via-primary/10 to-sky-500/10 border border-primary/20 flex items-center justify-center group-hover:border-primary/45 transition-colors">
          <span
      ...
      <nav className="flex items-center gap-0.5 sm:gap-1 flex-1 min-w-0 overflow-x-auto py-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="flex items-center gap-0.5 bg-surface-container/50 rounded-full p-0.5 border border-outline-variant/10 shrink-0">
